...
ggplot(BMavesPrd, aes(x=FemurCirc, y= BM.g)) +
      geom_point(aes(color=Clade.1, shape = typeFC)) +
  geom_smooth()
...
library(splines)
BMSplinePrd <- lm(BM.g ~bs(FemurCirc, knots = 35), data = BMavesPrd)
BMSplineCVPrd <- train(BM.g ~bs(FemurCirc, knots = 35), data = BMavesPrd, method = "lm", trControl = ctrl)
...
```{r}
 ...
 AvesSex <- new |> filter(!is.na(Sex) & !is.na(TarsusLen) & Aves == "Y")
 
 ggplot(AvesSex, aes(x = Sex, y = TarsusLen)) +
   geom_boxplot()
 
 ggplot(AvesSex, aes(y = Sex, x = TarsusLen)) +
